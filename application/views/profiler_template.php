...
<div class="profile-div <?php echo date("His"); ?>">
	<style type="text/css">
		#codeigniter-profiler { clear: both; background: #222; padding: 0 5px; font-family: Helvetica, sans-serif; font-size: 10px !important; line-height: 12px; position: absolute; width: auto; min-width: 74em; max-width: 90%; z-index: 1000; display: none; z-index:9999999}
			...
			<div id="ci-profiler-console" class="ci-profiler-box" style="display: none">
				...
				<?php if (is_array($sections['console'])) : ?>
					...
					<?php foreach ($sections['console']['console'] as $log) : ?>
						...
						<?php if ($log['type'] == 'log') : ?>
							<tr>
								<td class="hilight"><?php echo $log['type'] ?> <small><br><?php echo date("H:m:s"); ?></small></td>
								<td class="faded"><pre style="-webkit-user-select: text;"><?php echo $log['data'] ?></pre></td>
								<td></td>
							</tr>
				...
				<?php if (is_array($sections['queries'])) : ?>
					...
					<?php foreach ($sections['queries'] as $key => $queries) : ?>
						<?php foreach ($queries as $time => $query): ?>
							<tr><td class="hilight"><?php echo $time ?></td><td style="-webkit-user-select: text;"><?php echo $query ?></td></tr>
						<?php endforeach; ?>
					<?php endforeach; ?>
					</table>
...
<script type="text/javascript" id="profile-js">
<?php if(!is_ajax()){ ?> $(function () { <?php } ?>
	

	console.info("Debug....");
	if($(".profile-div").length >1)
	{
		console.info("New Console");
		$(".profile-div:last-child").addClass("asdasd");
		$(".profile-div:last-child").remove();
	}
	$("#profile-js").remove();
	ci_profiler_bar.set_load_state();
<?php if(!is_ajax()){ ?> }); <?php } ?>
